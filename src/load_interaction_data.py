...
import re
from utils import sorted_nicely, get_data_path, assert_valid_nid, load, get_sparse_mat, save_klepto
...
def load_interaction_data(name, natts, eatts, tvt, align_metric, node_ordering, glabel):
    if 'drugbank' in name:
        dir_name = 'DrugBank'
        drugbank_dir = join(get_data_path(), dir_name)
        interaction_dir = join(drugbank_dir, 'DDI_datasets')
        ...
        edge_types_edge_list, nodes = get_interaction_edgelist_drugbank(interaction_file_path, parse_edge_func, eatts, fname_to_gid_func)
    ...
    return BiLevelDataset(name, graphs, natts, eatts, pairs, tvt, align_metric, node_ordering, glabel,
...
def get_drugbank_node_feats(graph_data, gids, fname_to_gid_func):
    feat_shape = len(list(graph_data.values())[0]["drug_feat"])
    gid_to_idx = {gid: i for i, gid in enumerate(sorted(list(gids)))}
    mat = np.zeros((len(gids), feat_shape))
    gid_graph_data = {fname_to_gid_func(id): g_data for id, g_data in graph_data.items()}
    for gid in gids:
        mat[gid_to_idx[gid]] = gid_graph_data[gid]["drug_feat"]
    return csr_matrix(mat), gid_to_idx
...
def get_interaction_edgelist_drugbank(file_path, parse_edges_func, eatts, edge_to_gid_func):
    ...
    with open(file_path, 'r') as f:
        for i, line in enumerate(f.readlines()):
            ...
            edge, edge_type = parse_edges_func(line)
            if edge:
