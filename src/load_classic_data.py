...
def load_debug_data(name, natts, eatts, tvt, align_metric, node_ordering, glabel):
    ...
    if name in ['debug', 'debug_no-1']:
        ...
        for _ in range(train_size):
            ...
            assert len(mapping_train) == g1.number_of_nodes()
            ...
            train_pairs[(num_graphs, num_graphs+1)] = GraphPair([mapping_train], ds_true=g1.number_of_nodes(), running_time=0)
            num_graphs += 2
        ...
        for _ in range(test_size):
            ...
            test_pairs[(num_graphs, num_graphs+1)] = GraphPair([mapping_test], ds_true=g1.number_of_nodes(), running_time=0)
            num_graphs += 2
...
def load_classic_data(name, natts, eatts, tvt, align_metric, node_ordering, glabel):
    node_labels = False
    if name == 'aids700nef':
        dir_name = 'AIDS700nef'
        ...
        dir_name = 'PTC'
        node_labels = True
        assert glabel is None  # TODO for Joseph: load glabels for ptc
    elif name == 'webeasy':
        dir_name = 'WEBEASY'
        node_labels = True
        assert glabel is None  # TODO for Joseph: load glabels
    elif name == 'nci109':
        dir_name = 'NCI109'
        node_labels = True
        assert glabel is None  # TODO for Joseph: load glabels
