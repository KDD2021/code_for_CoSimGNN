...
def format_mseconds(mseconds):
    ...
    if mseconds <= 1:
        return '{:.2f} msecs'.format(mseconds)

    strings = []
    for period_name, period_seconds in periods:
        if mseconds > period_seconds:
            if period_name == 'msec':
                period_value = mseconds
                has_s = 's'
            else:
