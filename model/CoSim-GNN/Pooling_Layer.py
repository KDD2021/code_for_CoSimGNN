...
class Pooling_Layers(nn.Module):
	...
	def __init__(self, Heads, Dim_input, N_output, Dim_output, CosimGNN=True, max_num_nodes=400, with_mask=False, Tau=1,
		...
		"""
		super(Pooling_Layers, self).__init__()
		self.Heads = Heads
		self.Tau = Tau
	...
	def forward(self, ins, batch_data, model):
		...
		for i in range(0, len(node_set_input), 2):
			...
			if self.CosimGNN:
				new_node_set, new_adj = self.Adaptive_Pooling(node_set, adj, zero_tensor)
			else:
	...
	def Adaptive_Pooling(self, node_set, adj, zero_tensor):
		"""
			node_set: Input node set in form of [batch_size, N_input, Dim_input]
			adj: adjacency matrix for node set x in form of [batch_size, N_input, N_input]
			zero_tensor: zero_tensor of size [1]
